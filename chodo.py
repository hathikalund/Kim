...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "Mood ni hai abhi pelhe purchase kar isse:- @HMSahil."
...
 Usage: /remove <userid>'''
        ...
        response = "Purchase karle bsdk:- @HMSahil ."
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 121:
                response = "Error: Buy From @HMSahil For More Time"
            else:
...
'''
    for handler in bot.message_handlers:
        if hasattr(handler, 'commands'):
            if message.text.startswith('/help'): to 
